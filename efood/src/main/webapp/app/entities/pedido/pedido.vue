<template>
  <div>
    <h2 id="page-heading" data-cy="PedidoHeading">
      <span v-text="$t('efoodApp.pedido.home.title')" id="pedido-heading">Pedidos</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('efoodApp.pedido.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PedidoCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('efoodApp.pedido.home.createLabel')"> Create a new Pedido </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && pedidos && pedidos.length === 0">
      <span v-text="$t('efoodApp.pedido.home.notFound')">No pedidos found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="pedidos">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('efoodApp.pedido.nomePedido')">Nome Pedido</span></th>
            <th scope="row"><span v-text="$t('efoodApp.pedido.custoPedido')">Custo Pedido</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="efoodApp.pedido.delete.question" data-cy="pedidoDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-pedido-heading" v-text="$t('efoodApp.pedido.delete.question', { id: removeId })">
