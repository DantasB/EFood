<template>
  <div>
    <h2 id="page-heading" data-cy="UsuarioHeading">
      <span v-text="$t('efoodApp.usuario.home.title')" id="usuario-heading">Usuarios</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('efoodApp.usuario.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'UsuarioCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('efoodApp.usuario.home.createLabel')"> Create a new Usuario </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && usuarios && usuarios.length === 0">
      <span v-text="$t('efoodApp.usuario.home.notFound')">No usuarios found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="usuarios">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('efoodApp.usuario.nomeUsuario')">Nome Usuario</span></th>
            <th scope="row"><span v-text="$t('efoodApp.usuario.enderecoUsuario')">Endereco Usuario</span></th>
            <th scope="row"><span v-text="$t('efoodApp.usuario.emailUsuario')">Email Usuario</span></th>
            <th scope="row"><span v-text="$t('efoodApp.usuario.telefoneUsuario')">Telefone Usuario</span></th>
            <th scope="row"><span v-text="$t('efoodApp.usuario.cpfUsuario')">Cpf Usuario</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="efoodApp.usuario.delete.question" data-cy="usuarioDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-usuario-heading" v-text="$t('efoodApp.usuario.delete.question', { id: removeId })">
